...
import { isEqual, sortBy, keyBy, range } from "lodash-es"
...
class Runner {
  async carregaPdf(buffer: Buffer): Promise<PdfDocument> {
    const pdfParser = new Pdf()
    return new Promise((resolve, reject) => {
      pdfParser.on("pdfParser_dataReady", (data: any) => {
        resolve(data)
      })
      pdfParser.on("pdfParser._dataError", (err: any) => {
        reject(err)
      })
      ...
      files.map(async fileName => {
        ...
        const entries = this.obterLancamentosDoDocumento(new DocumentWrap(pdfInfo), fileName)
          .filter(entry => {
            ...
            if (entry.lancamento === "Saldo Anterior") return false
            if (entry.dia === "Dia") return false
            return true
          })
          .map(entry => {
  ...
  obterLancamentosDoDocumento(document: DocumentWrap, nomeArquivo: string) {
    const lancamentos = document.findText("Lançamentos").next().value!
    const tableRangeAtPage =
      (checkPage: number) =>
      ({ path }) => {
        const nodePage = path.at(1) as number
        if (nodePage !== checkPage) return false
        const index = Number(path.at(-1))
        if (index <= Number(lancamentos.path.at(-1))) return false
        return true
      }
    ...
    const porPagina = range(paginasDeSaldo).map(pagina => {
      ...
      const nodosTabelaIt = document.findText(null, ({ node, path }: SearchArg) => {
        const page = path[1] as number
        if (page !== pagina) return false

        if (node.y < dia.node.y) return false
        if (ehUltimaPagina) {
          if (node.y > marcadorDeFim.node.y) return false
        }
        if (isEqual(path, lancamentos.path)) return false
        if (isEqual(path, marcadorDeFim.path)) return false
        return true
      })
      ...
      for (const nodo of nodosTabela) {
        ...
        if (nodo.node.x === dia.node.x) {
          ...
          if (!pos.lancamento) pos.lancamento = nodo.textContent
          else {
            pos.lancDetalhe += nodo.textContent
          }
        } /* if (nodo.node.x === valor.node.x) */ else {
...
class DocumentWrap {
  ...
  *findText(
    text: string | null,
    constraint?: (ctx: { node: TextNode; path: Array<string | number> }) => boolean
  ) {
